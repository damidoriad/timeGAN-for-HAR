...
class Time_GAN_module(nn.Module):
    ...
    def forward(self, x):
            ...
            if self.rnn_type in ["rnn", "gru"]:
              ...
              h0 = torch.zeros(self.n_layers, x.size(0), self.hidden_dim).to(device).float()
              c0 = torch.zeros(self.n_layers, x.size(0), self.hidden_dim).to(device).float()
              hidden = (h0, c0)
...
def TimeGAN(data, parameters):
  ...
  for e in range(epoch): 
    for batch_index, X in enumerate(loader):
        ...
        X_tilde = torch.reshape(X_tilde.float(), (batch_size, seq_len, dim))

        # constants chosen like in the paper
        E_loss0 = 10 * torch.sqrt(MSE_loss(X, X_tilde))
        E_loss0 = E_loss0.float()  
